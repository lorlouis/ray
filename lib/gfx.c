...
Display *gfx_current_display() {
    return gfx_display;
}

Window gfx_current_window() {
    return gfx_window;
}
...
/* get the last key or button pressed since the last flush */
char gfx_get()
{
	XEvent event;
    while(XCheckWindowEvent(gfx_display, gfx_window, ButtonPressMask | KeyPressMask, &event)) {
        if(event.type==KeyPress) {
            saved_xpos = event.xkey.x;
            saved_ypos = event.xkey.y;
            gfx_flush();
